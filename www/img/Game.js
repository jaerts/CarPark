...
function onDragStop(_sprite, pointer) {
    ...
    var xx = _sprite.x;   
    xx = Math.floor((xx+CellSize/2)/CellSize) * CellSize;
    //_sprite.x = x;
	...
	if (a == 4 && b == 2 && car[_sprite.carindex].nummer == 'A') 
	{
		xx = 600;
		movetime = 2000;
		tween = this.game.add.tween(_sprite).to( { x: xx}, movetime, Phaser.Easing.Cubic.InOut, true);
	...
	{
		this.game.add.tween(_sprite).to( { x: xx}, movetime, Phaser.Easing.Cubic.InOut, true);
...
function onLevelComplete()
{
	...
	textMinMoves.text  = "minimal moves " + minMoves;
	moves = 0;
...
function DrawScreen() {
	...
	b_prev = game.add.sprite(1 * this.game.width/6, centerY, 'b_prev');
	...
	b_next = game.add.sprite(2 * this.game.width/6, centerY, 'b_next');
	...
	b_restart = game.add.sprite(3 * this.game.width/6, centerY, 'b_restart');
	...
	b_list = game.add.sprite(4 * this.game.width/6, centerY, 'b_list');
	...
	b_return = game.add.sprite(5 * this.game.width/6, centerY, 'b_return');
